  ...
  public function add( $key,  $data )
  {
    ...
    if(!is_dir($path))
    {
      if(!SFilesystem::createFolder($path))
      {
        ...
        (
          I18n::s( 'Failed to create the cache Folder', 'wbf.error.failedToWriteInCache' , array($key) )
        );
      }
    }
    ...
    if( !SFiles::writeCache( $this->folder.'/'.$key,  $data ) )
    {
      throw new LibCache_Exception
      (
        I18n::s( 'Failed to write in the Cache', 'wbf.error.failedToWriteInCache' , array($key) )
      );
    }
