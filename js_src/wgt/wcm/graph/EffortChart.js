  ...
  var drawChart = function(data) {
      ...
      graphData.map(function(series) {
		series.values.map(function(d) {
		    ...
		    // Don't convert two times
		    if(typeof(d.x) != "object") {
			d.x = inputDateFormat(d.x);
		    }
		});
		// Find max of all series
		seriesMax.push(d3.max(series.values, function(d) { return d.y }));
	} );
      ...
      var maxAvailableWidth = document.body.offsetWidth - 250;
      var maxAvailableHeight = document.body.offsetHeight - 200;
      
      // Maximum width of a bar
      var maxBarWidth = 30
        
      var margin = {top: 30, right: 100, bottom: 100, left: 50},
          width = maxAvailableWidth - margin.left - margin.right,
          height = maxAvailableHeight - margin.top - margin.bottom;
