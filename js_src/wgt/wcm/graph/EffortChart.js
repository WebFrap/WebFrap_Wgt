  ...
  // Setup data
 	...
 	var innerHeight = window.innerHeight - 450;
        ...
        var inputDateFormat = d3.time.format("%Y-%m-%d").parse;
      	...
      	var barSeries = layers.data.filter(function(d) {return d.type == "bar"} );
      	
      	var lineSeries = layers.data.filter(function(d) {return d.type == "line"} );
      	...
      	barSeries.map(function(bar, i) {
      	   	...
      	   	bar.series.map(function(d, j) {
      	   	    barData[i].values.push( {"x" : graphCategory[j], "y" : d} );
      	   	});
      	...
      	lineSeries.map(function(line, i) {
      	   	...
      	   	line.series.map(function(d, j) {
      	   	 	lineData[i].values.push( {"x" : graphCategory[j], "y" : d} );
      	   	});
      ...
      var stack = d3.layout.stack()
      	...
      	if(isDataAvailiable) {
      	    ...
      	    var chart = d3.select("#wgt-chart-project_effort_chart")
      		    .append("svg")
      		    .attr("class", "chart")
      		...
      		chart.selectAll(".actual")
      		    ...
      		    .style("fill", function(d) { return actual.color; } );
      	    ...
      	    chart.selectAll(".plan")
      		    ...
      		    .attr("y", function(d) { return yScale(d.y + d.y0); } )
      		    .attr("height", function(d) { return height - yScale(d.y); } )
      		    .style("fill", function(d) { return plan.color; } );
      	    
      	  //#######################################################################
      	  // Adjust bar witdh if necessary
      	  //#######################################################################
      	    ...
      	    chart.append("path")
      		    ...
      		    .style("stroke", function(d) { return demand.color; })
      		    .style("opacity", "0.8");
