...
$.fn.fullCalendar = function(options) {
	...
	this.each(function() {
		...
		function render(inc) {
		  view.trigger('beforeChangeDisplay', _element); //Added by milos kosanovic
			if (elementVisible()) {
		...
		function fetchEventSource(src, callback) {
			...
			if (typeof src == 'string') {
				...
				/**Added by milos kosanovic
				 * I extended the call so I can call when source begins
				 * with ahax.php
				 */
				if (src.substring(0,8) === 'ajax.php') {
				  
				  var newSrc = src + '&start=' + params[options.startParam] + 
                        '&end=' + params[options.endParam];
				  var data = $R.get(newSrc).data;
				  //alert(data);
                  if (data.length > 0){             
                    data = window.$B.robustParseJSON(data);
                  }
                  else {
                    data = new Array();
                  }
				  reportEventsAndPop(data);
				} /** end by milos kosanovic */
				else {
				  $.ajax({
					url: src,
					dataType: 'json',
		...
		var publicMethods = {
			...
			today: function() {
				date = new Date();
				date.setHours(0,0,0,0); //added by milos kosanovic
				render();
			},
			...
			renderEvent: function(event, stick) { // render a new event
				normalizeEvent(event, options);
				//if (!event.source) { //changed by milos kosanovic
				if (!event.source || event.newEvent){
					if (stick) {
		...
		if (!options.disableDragging && eventElement.draggable) {
			...
			eventElement.draggable({
				...
				stop: function(ev, ui) {
					...
					if (dayDelta) {
						eventElement.find('a').removeProp('href'); // prevents safari from visiting the link
	...
	function slotSegHtml(event, seg, className) {
		return "<div class='" + className + event.className.join(' ') + "' style='position:absolute;z-index:8;top:" + seg.top + "px;left:" + seg.left + "px'>" +
			"<a" + (event.url ? " href='" + htmlEscape(event.url) + "'" : '') + ">" +
				...
				"<span class='fc-event-time'>" + htmlEscape(formatDates(event.start, event.end, view.option('timeFormat'))) + 
				"</span>" +
				"<span class='fc-event-title'>" + htmlEscape(event.title) + "</span>" +
			"</a>" +
			((event.editable || event.editable === undefined && options.editable) && !options.disableResizing && $.fn.resizable ?
